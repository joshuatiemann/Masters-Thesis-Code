...
from transforms import Notransform, joint_centipede_graph, leg_centipede_graph
import actors
...
def run_experiment(config):
    ...
    transform_map = {
        ...
        "joint_centipede_graph": joint_centipede_graph,
        "leg_centipede_graph": leg_centipede_graph,
    }

    actor_map = {
        "mlp_actor": lambda: actors.mlp_actor(num_cells=128, action_dim=16, device=device),
    ...
    observation_shape = (111,) if config.contact_forces else (71,)
    ...
    if video:
        ...
        if "observation" in video_rollout:
            ...
            np.savetxt(f"{file_path}/video_observations.txt", observations[:,], delimiter=",")
...
if __name__ == "__main__":
    ...
    experiment_list = [leg_config, mlp_config]
