...
#corresponds to the baseline graph, where each actuators is represented by a single node + torso node, edges resulting from body connectivity
...
class joint_graph(ObservationTransform):
    def _apply_transform(self, obs: torch.Tensor) -> NonTensorData:
        data = Data()
        data.x = torch.tensor([[obs[0], obs[1], obs[2], obs[3], obs[4], obs[13], obs[14], obs[15], obs[16], obs[17], obs[18]],#torso
                                [obs[5], obs[19],0,0,0,0,0,0,0,0,0], #hip_1 front left
