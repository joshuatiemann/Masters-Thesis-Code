...
from torch_geometric.nn import Linear, Sequential, GCNConv, GraphConv, MessagePassing, global_mean_pool, HGTConv, HeteroConv, to_hetero, GATv2Conv
from torch_geometric.data import Data, Batch
    
def mlp_actor(num_cells=128, action_dim=8, hidden_layer=2, device="cpu"):
...
class joint_actor(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.joint_lin = nn.Sequential(Linear(2, 64))
        self.torso_lin = nn.Sequential(Linear(11, 64))
    ...
    def forward(self, data):
        ...
        outputs = []
        for i in range(8):
            outputs.append(self.output_layer[i](joints[:, i, :]))
...
class leg_actor(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.joint_lin = nn.Sequential(Linear(4, 64))
        self.torso_lin = nn.Sequential(Linear(11, 64))
        self.propagation_model = Sequential("x, edge_index", [
            (GraphConv(64, 64), "x, edge_index -> x"),
            nn.Tanh(),
